...
public class DLXSudokuSamurai extends AbstractSudokuSolver
{
	...
	protected static int getIdx(int row, int col, int num)
	{
		...
		if(row <= 9 && col <= 9)
		{
			//topleft sudoku. no offset needed assign   0-80
		}
		else if(row <= 9 && 13 <= col)
		{
			//topright sudoku                  assign  81-161
			cellOffset = 81;
			colOffset = 12;
		}
		else if(13 <= row && col <= 9)
		{
			//bottomleft sudoku                assign 162-242
			cellOffset = 2 * 81;
			rowOffset = 12;
		}
		else if(13 <= row && 13 <= col)
		{
			//bottomright sudoku                assign 243-323
			cellOffset = 3 * 81;
			rowOffset = 12;
			colOffset = 12;
		...
		return cellOffset * 9 + (row - rowOffset - 1) * nrCols * nrSymbols + (col - colOffset - 1) * nrSymbols + (num - 1);
	}
