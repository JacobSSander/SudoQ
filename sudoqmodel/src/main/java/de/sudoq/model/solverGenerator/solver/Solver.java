...
public class Solver
{
	 ...
	 *
	 * @param applySolution Gibt an, ob die Lösung direkt in das Feld eingetragen werden soll oder nicht
	...
	public Solution solveOne(boolean applySolution)
	{
		...
		while(!solvedField && didUpdate && !isIncorrect)
		{
			...
			{
				return null; // if every field is already filled, no solution can be found, because that already happened
			}
			...
			if(isInvalid())
			{
				...
				{
					// if sudoku is invalid && has no branches we're in a dead end
					isIncorrect = true;
				}
				else
				{
			...
			for(Position p : this.sudoku.positions)
			{
				...
				if(b.cardinality() == 1)
				{ //we found a field where only one note remains
					if(!this.sudoku.hasBranch())
					{
	 ...
	 *
	 * @param buildDerivation Gibt an, ob eine Herleitung der Lösung erstellt werden soll
	 * @param applySolutions Gibt an, ob die Lösungen direkt in das Sudoku eingetragen werden sollen
	...
	public String getHintCountString()
	{
		...
		{
			countsBuilder.append(separator).append(em.get(h) + " " + h);
	 ...
	 *
	 * @param solution In diese Map wird die ermittelte Lösung geschrieben
	...
	public ComplexityRelation validate(PositionMap<Integer> solution)
	{
		...
		if(solved)
		{
			...
			if(maxComplextiy * 1.2 < complexity)
			{
				result = ComplexityRelation.MUCH_TOO_DIFFICULT;
			}
			...
			else if(complexity <= minComplextiy * 0.8)
			{
				result = ComplexityRelation.MUCH_TOO_EASY;
			}
	 ...
	 *
	 * @param buildDerivation Bestimmt, ob die Herleitung der Lösung oder lediglich eine leere Liste zurückgegeben werden soll
	 * @param followComplexityConstraints Bestimmt, ob zum Lösen die Constraints der Komplexität des Sudokus befolgt werden müssen
	 * @param validation
 ...
 */
	 ...
	 *
	 * @param addDerivations Bestimmt, ob die Herleitung der Lösungen zurückgegeben oder lediglich eine leere Liste zurückgegeben werden soll
	 * @param addComplexity Bestimmt, ob der Schwierigkeitswert beim Finden eines NakedSingles dem Sudoku hinzugefügt werden soll
			...
			for(Position p : this.sudoku.positions)
			{
				...
				if(b.cardinality() == 1)
				{
					if(addDerivations)
					{
						...
						deriv.addDerivationCell(new DerivationCell(p, (BitSet) b.clone(), new BitSet()));
