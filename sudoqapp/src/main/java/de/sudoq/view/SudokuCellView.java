...
public class SudokuCellView extends View implements ModelChangeListener<Cell>, ObservableCellInteraction
{
	 ...
	 * @param game the game this view is a part of
	 * @param cell The cell represented by this SudokuCellView
	 * @param markWrongSymbol indicates if an incorrect value should be highlighted eoll
	...
	public SudokuCellView(final Context context, Game game, Cell cell, boolean markWrongSymbol)
	{
		...
		{
			if(c.getType().equals(ConstraintType.EXTRA) && c.includes(game.getSudoku().getPosition(cell.getId())))
			{
	 ...
	 * @throws IllegalArgumentException if canvas is null TODO really??
	 */
	@Override
	 ...
	 *
	 * @param touchEvent Das TouchEvent das von der API kommt und diese Methode aufgerufen hat
	 * @return true falls das TouchEvent behandelt wurde, false falls nicht
	 * @throws IllegalArgumentException Wird geworfen, falls das übergebene MotionEvent null ist
	 */
	/*@Override
	 ...
	 *
	 * @param state true, um den Notizmodus ein-, bzw. false um ihn auszuschalten
	 */
	 ...
	 *
	 * @param view the View to connect with this one
	 */
	 ...
	 *
	 * @param markConnected Determines if cells connected to this cell (row / column) should be highlighted
	 */
	 ...
	 *
	 * @param updateConnected determines if connected cells should also be reset.
	 */
			...
			if(c.includes(sudoku.getPosition(this.cell.getId())))
			{
				if(c.hasUniqueBehavior())
				{
					for(Position pos : c.getPositions())
					{
						...
						if(pos != sudoku.getPosition(this.cell.getId()) && sudoku.getCell(pos).getCurrentValue() == this.cell.getCurrentValue())
						{
							return true;
						}
					}
